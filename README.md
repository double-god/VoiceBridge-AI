...
本项目构建了一个智能体（Agent），利用 **OpenAI Whisper** 进行本地转录，结合 **大语言模型 (LLM)** 的语义推理能力，并参考用户的**个人画像 (Persona)** 与**环境语境**，将破碎、模糊的语音"修复"为清晰的意图，最终通过 **CosyVoice** 合成清晰的语音代替用户表达。
...
- **🔊 清晰语音播报**: 集成 TTS（CosyVoice），支持清晰响亮的语音输出。
